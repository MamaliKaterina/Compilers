...
and compile_expr info ast =
  ...
  | E_is_nil (e, line)          -> (let t = compile_expr info e in
                                   try
                                     ignore (Llvm.struct_name (Llvm.pointer_type (Llvm.type_of t)));
  ...
  | E_cons (e1, e2, line)       -> (let v1 = compile_expr info e1
                                   and v2 = compile_expr info e2 in
                                       try
                                         let struct_t = Llvm.pointer_type (Llvm.type_of v2) in
...
and compile_atom info ast =
  ...
  | A_atom (a, e, line) -> let v = compile_atom info a
                           ...
                           begin
                           try
                             ignore (Llvm.pointer_type (Llvm.pointer_type (Llvm.type_of v)));
...
and compile_simple info ast =
  ...
  | S_assign (a, e, line) -> let x = compile_atom info a
                             ...
                             if (Llvm.pointer_type (Llvm.type_of x)) <> (Llvm.type_of y) then (error "lvalue and rvalue in assignment are not of the same type";
                                                                                raise (TypeError line))
